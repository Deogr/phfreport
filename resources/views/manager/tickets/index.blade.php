<x-app-layout>
    ...
    <div class="py-12" x-data="{ 
        ...
        openEdit(ticket) {
            ...
            this.editUrl = '{{ route('manager.tickets.update', ':id') }}'.replace(':id', ticket.id);
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center p-1 bg-gray-100 dark:bg-gray-700/50 rounded-lg">
                    ...
                    <a href="{{ route('manager.tickets', ['status' => 'all']) }}"
                        class="px-4 py-2 text-sm font-bold rounded-md transition-all duration-200 {{ $currentStatus === 'all' ? 'bg-white dark:bg-gray-800 text-primary shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200' }}">
                        All <span
                            class="ml-1 px-2 py-0.5 text-[10px] bg-gray-200 dark:bg-gray-700 rounded-full">{{ $counts['all'] }}</span>
                    </a>
                    <a href="{{ route('manager.tickets', ['status' => 'paid']) }}"
                        class="px-4 py-2 text-sm font-bold rounded-md transition-all duration-200 {{ $currentStatus === 'paid' ? 'bg-white dark:bg-gray-800 text-primary shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200' }}">
                        Paid <span
                            class="ml-1 px-2 py-0.5 text-[10px] bg-green-100 text-green-700 rounded-full">{{ $counts['paid'] }}</span>
                    </a>
                    <a href="{{ route('manager.tickets', ['status' => 'used']) }}"
                        class="px-4 py-2 text-sm font-bold rounded-md transition-all duration-200 {{ $currentStatus === 'used' ? 'bg-white dark:bg-gray-800 text-primary shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200' }}">
                        Used <span
                            class="ml-1 px-2 py-0.5 text-[10px] bg-blue-100 text-blue-700 rounded-full">{{ $counts['used'] }}</span>
                    </a>
                    <a href="{{ route('manager.tickets', ['status' => 'cancelled']) }}"
                        class="px-4 py-2 text-sm font-bold rounded-md transition-all duration-200 {{ $currentStatus === 'cancelled' ? 'bg-white dark:bg-gray-800 text-primary shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200' }}">
                        Cancelled <span
                            class="ml-1 px-2 py-0.5 text-[10px] bg-red-100 text-red-700 rounded-full">{{ $counts['cancelled'] }}</span>
                    </a>
                </div>
                <div class="flex space-x-2">
                    <a href="{{ route('manager.tickets.items') }}"
                        class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow-sm transition-all duration-200">
                        View Item Registry
                    </a>
                    <button @click="showCreateModal = true"
                        class="bg-primary hover:bg-primary-hover text-white font-bold py-2 px-4 rounded shadow-sm transition-all duration-200">
                        Sell New Ticket(s)
                    </button>
                </div>
            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td
                                            ...
                                            @if($ticket->guest_phone)
                                                <span class="text-xs text-gray-400 block">{{ $ticket->guest_phone }}</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                            {{ ucfirst($ticket->type) }}
                                        ...
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 uppercase">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            @if($ticket->quantity > 1)
                                                <a href="{{ route('manager.tickets.export', $ticket) }}" target="_blank"
                                                    class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 mr-3">Export</a>
                                            @endif
                                            <a href="{{ route('manager.tickets.print', $ticket) }}" target="_blank"
                                            ...
                                            <form action="{{ route('manager.tickets.destroy', $ticket) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
