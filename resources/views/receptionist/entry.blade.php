    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <!-- Entry Form -->
                <div class="lg:col-span-2 bg-white dark:bg-surface-dark overflow-hidden shadow-sm sm:rounded-lg p-6 border border-gray-200 dark:border-gray-800">
                    <header class="flex items-center justify-between">
                        <div>
                            <h2 class="text-lg font-bold text-gray-900 dark:text-white">
                                {{ __('Record Attendance') }}
                            </h2>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Submit new transaction for the current station.</p>
                        </div>
                        @if($activeAssignment)
                            <div class="flex items-center gap-2 px-3 py-1 bg-green-50 dark:bg-green-900/20 rounded border border-green-100 dark:border-green-900/30">
                                <span class="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                                <span class="text-[10px] font-bold text-green-700 dark:text-green-400 uppercase tracking-widest">Assigned to: {{ $activeAssignment->station->name }}</span>
                            </div>
                        @endif
                    </header>
                    ...
                    <form method="post" action="{{ route('receptionist.store') }}" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="station_id" name="station_id"
                                    ...
                                    @else
                                        <option value="{{ $activeAssignment->station->id }}" selected>{{ $activeAssignment->station->name }}</option>
                                    @endif
                                </select>
                            ...
                            <!-- Service Selection -->
                            <div @if($isGym) class="hidden" @endif>
                                <x-input-label for="service_id" :value="__('Service / Entrance Type')" />
                                @if($isGym && $gymService)
                                    <input type="hidden" name="service_id" value="{{ $gymService->id }}">
                                    <div class="mt-1 p-2 bg-gray-50 dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400">
                                        {{ $gymService->name }} (Automatically Selected)
                                    </div>
                                @else
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div x-data="{ 
                                ...
                                get isVerified() {
                                    if (this.method === 'Cash' || this.method === 'Mobile') return true;
                                ...
                                <div>
                                    ...
                                    <select id="payment_method" name="payment_method" x-model="method"
                                        class="mt-1 block w-full border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-primary focus:ring-primary rounded-md shadow-sm"
                                        required @change="verificationResult = null; verifyCode = ''; message = ''; toggleAmountField(); if(method === 'Cash' || method === 'Mobile') { document.getElementById('amount').readOnly = false; }">
                                        @if($isGym)
                                ...
                                <!-- Verification Field -->
                                <div x-show="method === 'Signature' || method === 'Ticket' || method === 'Subscription'" class="mt-4 p-4 bg-primary/5 dark:bg-primary/10 rounded-lg border border-primary/20" style="display: none;">
                                    <div class="flex items-end gap-2">
                                        <div class="flex-1">
                                            <label class="block text-[10px] font-bold text-primary uppercase tracking-widest mb-1" x-text="method === 'Ticket' ? 'Enter Ticket Code' : 'Enter Subscription ID / Phone'"></label>
                                        ...
                                        <template x-if="verificationResult">
                                            <div class="flex items-start gap-2 text-green-600 dark:text-green-400">
                                                ...
                                                <div class="text-xs">
                                                    ...
                                                    <p x-show="verificationResult.expires" class="mt-1 text-[10px] opacity-75">Valid until: <span x-text="verificationResult.expires"></span></p>
                                                    <input type="hidden" name="subscription_id" :value="verificationResult.type === 'subscription' ? verificationResult.id : ''">
                                                    <input type="hidden" name="ticket_item_id" :value="verificationResult.type === 'ticket' ? verificationResult.id : ''">
                                                </div>
                                            </div>
                                        </template>
                                ...
                                <div class="flex flex-col md:flex-row md:items-center justify-between pt-6 border-t border-gray-100 dark:border-gray-800 mt-6 col-span-full gap-4">
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        @if (session('success'))
                                            <div class="flex items-center gap-1 text-green-600 font-bold text-xs uppercase tracking-wide px-3 h-11">
                                    ...
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center md:text-right" x-show="!isVerified">
                            ...
                            <div id="amount-field">
                                ...
                                <x-text-input id="amount" name="amount" type="number" class="mt-1 block w-full bg-gray-50/50 dark:bg-gray-800/50"
